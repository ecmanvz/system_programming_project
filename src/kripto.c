...
int main(int argc, char**argv ){
    ...
    JRB kilitAgac; // burda kilit dosyasının verilerini yazacağımız red-black ağacını tanımladık 
    kilitAgac = make_jrb();// boş ağaç oluşturduk ve kilitAgac'ın içine atadık

    

    if(strcmp(argv[1], D_PARAMETRESI) == 0){// eğer -d paremetresi verdiysek burası çalışıcak
       ...
       while (get_line(giris_Dosyasi) >= 0)// griş dosasini okuyoruz ve ağacın içinde arama yapıyoruz
       {
        ...
        jettison_inputstruct(kilit);// okuduğumuz dosyaları bellekten kaldırıyoruz ve çıkış yapıyoruz
